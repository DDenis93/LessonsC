...
void test1(int mas[],int n){
	// Проверка правильности сортировки - смотрим, чтобы все элементы были в порядке неубывания
	int x=mas[0];
	for(int i=1;i<n;i++){
		if(x>mas[i]){
...
// функция сравнения двух элеменотв cmp (от слова compare - "сравнивать")
int cmp(const void *p1,const void *p2){
	int i1=*(int*)p1;
	int i2=*(int*)p2;
	return i1<i2 ? -1:(i1==i2 ? 0:1);
}
// Библиотечная функция сортировки - qsort (quick sort)
